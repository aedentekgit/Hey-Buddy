...
from config import REALTIME_CHAT_ADDENDUM, GROQ_API_KEYS, GROQ_MODEL, GEMINI_API_KEY, OPENAI_API_KEY
...
class RealtimeGroqService(GroqService):
        ...
        if api_key and provider:
            if provider.lower() == 'gemini':
                ...
                self._fast_llm = ChatGoogleGenerativeAI(
                    ...
                    temperature=0.0,
                    timeout=GROQ_REQUEST_TIMEOUT_FAST,
                    max_tokens=50,
                )
    ...
    def get_response(self, question: str, chat_history: Optional[List[tuple]] = None, user_id: Optional[str] = None, memory_context: str = "") -> str:
        """
        ...
        try:
            ...
            t0 = time.perf_counter()
            response_content = self._invoke_llm(prompt, messages, question, system_message, user_id=user_id)
    ...
    def stream_response(self, question: str, chat_history: Optional[List[tuple]] = None, user_id: Optional[str] = None, memory_context: str = "") -> Iterator[Any]:
        """
