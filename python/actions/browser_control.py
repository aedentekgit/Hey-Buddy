...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".buddy_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".buddy_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Buddy profile")
                buddy = str(Path.home() / ".buddy_profiles" / "firefox_buddy")
                Path(buddy).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(buddy, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".buddy_profiles" / "safari")
        ...
        buddy_profile = str(Path.home() / ".buddy_profiles" / self.browser_name)
        Path(buddy_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Buddy profile: {buddy_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(buddy_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with Buddy profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "buddy_screenshot.png")
