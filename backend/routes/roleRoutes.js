...
const { getRoles, createRole, updateRole, deleteRole, assignRole } = require('../controllers/roleController');
const { protect, authorize } = require('../middlewares/auth');

router.route('/').get(protect, authorize('admin'), getRoles).post(protect, authorize('admin'), createRole);
router.route('/assign').post(protect, authorize('admin', 'Super Admin'), assignRole);
