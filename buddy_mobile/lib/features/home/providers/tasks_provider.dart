...
class TasksProvider with ChangeNotifier {
    ...
    try {
      ...
      for (int i = 0; i < _processedTasks.length; i++) {
        ...
        try {
          ...
          if (useLat != null && useLng != null && coords != null &&
            ...
            if (distKm > 1000) {
              print("[TasksProvider] GPS is ${distKm.toStringAsFixed(0)}km from destination — using Madurai fallback.");
          ...
          if (stats != null) {
            ...
            final durationSec = (stats['durationInTraffic'] as num?)?.toDouble() ?? 0;
  ...
  double _haversineKm(double lat1, double lng1, double lat2, double lng2) {
    ...
    final a = _sinSq(dLat / 2) +
        _cosDeg(lat1) * _cosDeg(lat2) * _sinSq(dLng / 2);
    final c = 2 * _asin(_sqrtClamp(a));
    return R * c;
  }

  // dart:math-free helpers (no import needed)
  double _sinSq(double x) { final s = x - x*x*x/6 + x*x*x*x*x/120; return s * s; }
  double _cosDeg(double deg) { final x = deg * 3.141592653589793 / 180; return 1 - x*x/2 + x*x*x*x/24; }
  double _sqrtClamp(double x) { if (x <= 0) return 0; if (x >= 1) return 1; double r = x; for (int i = 0; i < 10; i++) r = (r + x / r) / 2; return r; }
  double _asin(double x) { return x + x*x*x/6 + 3*x*x*x*x*x/40; }
